...
describes what users get. Pick a format with `-o`, or let usgsquake choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
usgsquake <command> -o table   # aligned columns for reading
usgsquake <command> -o jsonl   # one JSON object per line, for piping
usgsquake <command> -o json    # a single JSON array
usgsquake <command> -o csv     # spreadsheet friendly
usgsquake <command> -o tsv     # tab-separated
usgsquake <command> -o url     # just the URL column
usgsquake <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
usgsquake <command> --fields id,title,url
```
...
```bash
usgsquake <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
usgsquake <command>            # a table, because this is a terminal
usgsquake <command> | wc -l    # JSONL, because this is a pipe
```
